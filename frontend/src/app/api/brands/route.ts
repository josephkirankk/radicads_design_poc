...
export async function GET(request: Request) {
    try {
        console.log("[API Route] Fetching all brands");
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                { error: errorData.detail || "Failed to fetch brands" },
...
export async function POST(request: Request) {
    try {
        ...
        console.log("[API Route] Creating brand");
        console.log("[API Route] Backend URL:", BACKEND_URL);
        ...
        if (!response.ok) {
            ...
            console.error("[API Route] Backend error:", errorData);
            return NextResponse.json(
                { error: errorData.detail || "Failed to create brand" },
                { status: response.status }
            );
        }
