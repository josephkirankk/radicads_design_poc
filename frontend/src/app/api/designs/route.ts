...
export async function GET(request: Request) {
    try {
        console.log("[API Route] Fetching all designs");
        ...
        if (!response.ok) {
            ...
            return NextResponse.json(
                { error: errorData.detail || "Failed to fetch designs" },
...
export async function POST(request: Request) {
    try {
        ...
        console.log("[API Route] Creating design");
        console.log("[API Route] Backend URL:", BACKEND_URL);
        ...
        if (!response.ok) {
            ...
            console.error("[API Route] Backend error:", errorData);
            return NextResponse.json(
                { error: errorData.detail || "Failed to create design" },
                { status: response.status }
            );
        }
